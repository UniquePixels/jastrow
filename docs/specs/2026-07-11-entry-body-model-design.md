...
| 2026-08-05 | **B5 italic-marker extension implemented** (Task 15, per the 07 decision above): `splitLettered` recognizes two italic marker shapes alongside plain `a)` — the full pair `<i>a</i>)` (the 75 reviewed entries) and the span-end `a</i>)` where the letter closes a longer italic span (Q01353's `<i>section, a</i>)`; the span's `<i>` stays with the head). Each item records its raw marker so `joinLettered` stays byte-exact. Full-corpus gates re-run: all four round-trips 32,512/32,512; structural lettered splits 116 → 191; census/structural reconciliation now 189 + 2 = 191, zero unexplained (dry-run Finding 2 updated). Fixtures: O01078, P00480, Q01353 added to `lettered.jsonl` |
