...
| `senses` | Tree of `{label?, gloss, units[], senses[]}`. The first sense is unlabeled — it is the entry's intro flow, exactly as printed. `label` is normalized (`"—2)"` → `"2"`, `"a)"` → `"a"`); print punctuation regenerates by rule, byte-exact, else the entry is quarantined (B6) |
