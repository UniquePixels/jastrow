...
| Field | Notes |
|---|---|
| `id`, `slug`, `page` | Unchanged from parent §2.2 (identity, frozen URL slug, print locator via import rule 6 — the hOCR page index `data/page-index/entries.jsonl`). *(Read "baseline deployed files + the 107 hand edits" here until 2026-09-06; superseded with parent §6 rule 6, which those are now only a cross-check for.)* |
