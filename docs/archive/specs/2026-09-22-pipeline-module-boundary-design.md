...
| M9 | The patch records move to `admin/pipeline/patch/records/`. They exist only to turn raw source into entries — import definition, the same category as the transform rules, not data | keeping them in `data/`; or treating them as a project-wide artifact the admin tool would edit, which the maintainer ruled it will not |
