...
|---|---|---|
| `headword-unparsed` | `blocks` | the reader sees the raw string; under the §2 shape a true TEXT DEFECT (a `=` introducing a gloss reference, a Latin word that is no numeral) rather than a shape the grammar could not fit. 29 rows → 2 |
| `paren-group-close-unknown` | `defer` | the forms are clean and written; only the layout is unsettled, and headword-design §3 rules that display uncertainty does not block go-live |
| `headword-partial-only` | `note` | every alternate is printed short, so the entry has no alternate search key until [#107](https://github.com/UniquePixels/jastrow/issues/107). Nothing is wrong with the data; the row keeps the population counted |
| `headword-duplicate-form` | `defer` | the line names one form twice, compared in NFC. Neither §3 nor §4 rules on the shape, so it is reported rather than halted on |
...
[research-backlog.md](../research-backlog.md) Group A: 23 → 22 and
89 → 88.
...
| research notes filed beside the data: `data/patches/{catalogue-audit,discovery-round-2,discovery-round-3,checkpoints}/`, `data/source/divergence-report.json` | `docs/archive/` on `v2` | reports, not correction data or source data (§1.1); confirm no file is a `migrate` input before moving. Not `data/patches/pilot/`: `patch/apply.ts` loads it as carry-over patches. **Superseded 2026-09-22** — all three pilot patches were `superseded`, so the directory held nothing the run applies and moved to `docs/archive/patches-retired-2026-09-22/pilot/`; `PILOT_DIR` is gone from `apply.ts`. **Cost measured 2026-09-16:** 11 surviving source files cited paths inside `data/patches/catalogue-audit/` and were rewritten to `docs/archive/catalogue-audit/` in the same commit — the maintainer's decision, not an accident |
| `data/patches/tranches/` | stays | **Added 2026-09-16:** not named above. It is a production input, read by `patch/apply.ts` alongside `pilot/`; neither is archived. **Amended 2026-09-22:** `tranches/` stays, but only the records the run applies do — `pilot/` and 72 retired patch records left for `docs/archive/patches-retired-2026-09-22/` |
...
7. *Shipped (#95).* Slug freezing (§7): `data/slug-index/` seeded from the committed
   ...
   [`docs/archive/plans/2026-09-17-consolidation-step7.md`](../plans/2026-09-17-consolidation-step7.md).
...
11. *Shipped (#112).* Research backlog triage (added 2026-09-18).
    ...
    **Nine sense-structure classes defer with a precondition** (§10).
    ...
    [`docs/v2/research-backlog.md`](../research-backlog.md); the plan is
    [`docs/archive/plans/2026-09-20-consolidation-step11.md`](../plans/2026-09-20-consolidation-step11.md).
...
| 2026-09-21 | §7 retired in code by the [URL names spec](2026-09-21-url-names-design.md) §9 steps 1–3. `slug` is gone from the schema and `sefariaHeadword` is required; `data/slug-index/` (both files), `migrate/slug.ts`, `migrate/slug-index.ts`, `migrate/seed-slug-index.ts`, `SLUGS_FROZEN` and the six `slug-*` review kinds are deleted, and the README is archived at [`docs/archive/slug-index-README.md`](../slug-index-README.md). The `slugs` gate becomes the `names` gate — still nine — checking current-name uniqueness in NFC over the finished entries and `sefariaHeadword` against a fresh read of the snapshot. §7's body stays as the record of what was built, per this spec's own §2 rule on dated text. Steps 4–6 of that spec (the admin rename flow, compile's route map, the published-names ledger) are not built; `formerNames` is in the schema and nothing writes it |
| 2026-09-21 | **headword-design §2 adopted** ([#130](https://github.com/UniquePixels/jastrow/pull/130), HW-schema): entry files carry `headwords[]` (index 0 primary) and an optional `display` template in place of `headword`/`altHeadwords`, under `"schemaVersion": 2`, and §3.1's rules are enforced in `validate.ts`. One parser reads the whole headword LINE, because print's grouping, its `?` and its `…` sit BETWEEN the forms where the old per-item grammar had nowhere to put them. Gate 2 is redefined — byte regeneration is impossible once a group split across four source items is four forms and one template — as three marks: Hebrew conserved in order, notation multiset agreeing, and a template present exactly for a line the gate independently judges settleable. `parenthesized-alt-headword` and `phrase-alt-headword-stub` are UNREGISTERED, which moved the commutation `crossPhasePairs` baseline 336 → 322 (46 × 7, its first downward move) and `entangledClusters` 8 → 7; `registry.ts` gains a fourth coverage state, `RETIRED`, so a row a maintainer ruled out is distinguishable from a row nobody looked at. The `reform` payload becomes `forms` plus an optional `display` — the only way a layout the source cannot settle is ever written — and the 15 shipped records are read forward rather than rewritten under their author's name. Review kinds: `headword-multiword` retires, `paren-group-close-unknown`, `headword-partial-only` and `headword-duplicate-form` arrive. Measured: gate 2 97,501/97,501; `headword-unparsed` 29 → 2 (A01175, A01345, whose `=` needs the op #113 still owes); `paren-group-close-unknown` 33; `headword-partial-only` 1,597; `headword-duplicate-form` 4; `headword-multiword` 271 → 0. Two things this did NOT decide and left to the maintainer: the G00674/G00675 name collision the correction creates (gate 7 red at 65,023/65,024), and A02823's display, where §2's table gives print's placement and §5 rules that only the source's is recorded |
| 2026-09-22 | **The batched rewrite of `data/entries/` to schema v2** ([#131](https://github.com/UniquePixels/jastrow/pull/131)). One `--write` run carried every code change since 2026-09-09 into the tree: 32,512 files changed, none added or dropped, the rid set identical. `headwords[]` + optional `display` for `headword`/`altHeadwords` (8,677 had alternates; 2,275 entries carry a `partial` form, 808 a `disambiguator`), `sefariaHeadword` for `slug`, `"schemaVersion": 2` on all of them, 432 losing a `grammar` block to the `pr. n. pl.` fix ([#117](https://github.com/UniquePixels/jastrow/issues/117)) the tree predated, and `display` unset on 35 lines §3 will not guess (33 `paren-group-close-unknown`, 2 `headword-unparsed`). The two edges #130 left to the maintainer are closed by reviewed patches, ruled 2026-09-22: G00675 takes `disambiguator: 2` (gate 7 65,023 → 65,024/65,024) and A02823 carries the print's `({0}) {1} I`. P000300 is restated against the source block, which carries `(אַיְידִי)` again now that `parenthesized-alt-headword` is unregistered; `patches.applied` 229 → 232, `upstream-changed` 1 → 0. NFC on write ships with it ([#110](https://github.com/UniquePixels/jastrow/issues/110)): `normalizeForWrite` normalized 164 strings in 150 files, each under an `NFD(before) == NFD(after)` assertion, and a re-scan of the written tree finds 0 non-NFC strings. Two checks #130 shipped had to be corrected, both resting on the assumption that a `reform` always rewrites its forms block: the post-apply assertion now asks whether the apply changed ANYTHING, and gate 2 stops refusing a patch-supplied template on a settleable line, holding it to the line's notation multiset instead — a placement may be corrected, notation may not be invented. `truth.test.ts` loses its transitional shim, so the committed tree is validated whole. Nine gates green; review report 2 `blocks` (A01175, A01345), 2,811 `defer`, 1,597 `note`. A second and third dry run leave `git status` clean. |
| 2026-09-22 | **The pipeline is declared final.** The post-consolidation review's code and structure work landed across #116–#131 — the review itself and its four reports, the URL names spec and the schema-v2 headword shape, gate and detector work, the comment and decision sweeps, the entry rewrite — and this PR closes it out: 43 dead exports un-exported, docstring coverage 82.0% → 100% over 477 exports with four stale claims corrected, Q12's five length and complexity rules raised from `on` (info) to `warn` so the 105 findings they had been emitting silently now fail `--error-on-warnings`, and Q9's moves made: `body/types.ts` and `body/compose.ts` to the pipeline root, `buildTrace` extracted to `body/trace.ts` with the round-trip verifier as `body/round-trip.ts`, and the dry-run CLI, `dry-run-report.ts` and `page-index/verify.ts` archived to `refs/tags/archive/v2-research-2026-09`. No detector moved: none of the four the review named is detect-only by `detectors/row.ts`'s definition. Behaviour-neutral throughout — `transform:invariants` 11/11 with every baseline unmoved, and a dry `data:import` returns `migration-blessing.md` and `review-report.md` byte-identical. `compile.ts` is the next effort and a separate one |
| 2026-09-22 | **`patch:replay` deleted** (Brian's ruling): `patch/apply-cli.ts` and its test are gone, along with the `patch:replay` script — it duplicated `bun data:import`'s own patch preflight (§4.2's Duplication finding) and could not complete on the corpus, blocking on the ~600 `needs_*` escalations import deliberately defers under Ruling D's `defer` policy. `patch/apply.ts`'s `loadManifest` export, used externally only by `apply-cli.ts`, is un-exported; every other preflight helper it called (`corpusPreflight`, `loadAcceptedCorpus`, `loadReviewedCorpus`, `patchesByRid`, `reviewedManifestProblems`) is still `migrate.ts`'s, unchanged. §4.2's silent-skip gap this also surfaces — consolidation dropping 11 patches on 4 rids (A00339, A00337, A00878, A00074) with no row or count — gets its own report row, `patch-consolidated-away` |
| 2026-09-22 | **`data/patches/` holds only what the import run applies** (Brian's ruling). 72 patch records left for [`docs/archive/patches-retired-2026-09-22/`](../patches-retired-2026-09-22/): the 61 `superseded` carry-overs — a transform rule reaches each defect first, so the run absorbed them and applied nothing — and the 11 Rulings C/F dropped, which #134 had just made visible as `patch-consolidated-away` rows. `pilot/` moved whole, having held nothing the run applies, and `PILOT_DIR`/`PILOT_STAGE` went with it; the sweep-era residue (`verdicts-*.jsonl`, `rejects.jsonl`, `report*.md`, 31 files) moved too — `corpusFiles` reads only `patches.jsonl` and `manifest.jsonl`. **An escalation is not a patch:** 15 `needs_*` records that carried a retired patch stayed where they were and lost only the reference, so no escalation left `data/patches/` except the pilot's 10, which travelled with its directory. Measured: nine gates green, `applied` 232 unchanged, `carry-over absorbed` 61 → 0, `carried` 5, `consolidated away` 11 → 0, review report `defer` 2,822 → 2,811 with `blocks` and `note` unmoved and no other row touched. The control that makes "superseded" honest: `data/entries/` deleted and rebuilt by `bun data:import --write` with the 72 records gone returns a byte-identical tree — they changed nothing, which is why removing them is free |
