...
## Moved from registry.ts, 2026-09-21

`transform/registry.ts` was 77 % comment (1,433 of 1,843 lines), with
155–194 lines of dated self-correction. PR 9a rewrote the file's
comments to state the ordering constraints and their reasons in the
present tense, and lifted the narration here: the dated `CORRECTED` /
`MOVED` / `REORDERED` / `DECLARED` paragraphs, the per-batch task
citations, the recounts that went stale on a later batch, and the
per-rule measurement records that are evidence for a placement rather
than a constraint on it.

Nothing below is a live instruction. Where a paragraph here disagrees
with the code, the code is right — these are the working notes that
produced today's order, kept verbatim so the reasoning behind a
placement stays recoverable.

### The `RULES` array as it stood at `99ec9895`
...
### Docstrings compressed in the same pass

`Ordered`, `PENDING`, `COVERED`, `undirectedGraph`, `checkAdjacency`
and `unaccountedEdges` each carried a dated recount or a branch-by-
branch account of how the check reached its current shape. The
contracts stayed in the code; these are the paragraphs that went.
