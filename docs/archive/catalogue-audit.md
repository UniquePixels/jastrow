...
| `superscript-subsection-stranded-outside-anchor` (160) | ~~its 38 contradiction cases **carry their own correct answer** (printed superscript right in 9 of 12 adjudicated); deterministically fixable~~ **RETRACTED 2026-08-26 (impl/phase-2-batch-4).** 12 occurrences were adjudicated, not 38, and of those the print was wrong in 2 and undecidable in 1. The 38 occ / 33 ent are now `superscript-subsection-contradicts-link-sub-section`, **route `judgment`** by Brian's ruling of 2026-08-26: the deltas spread −17…+20 with modal −1 at 9 of 38, so nothing mechanical recovers the value. The parent ships a BOUNDARY MOVE only (`superscriptInsideAnchor`); **no shipped rule writes a sub-section into a `data-ref`**. |
