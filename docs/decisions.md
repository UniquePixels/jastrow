...
**This file holds only what still binds the pipeline.** On 2026-09-22
every row was classified by grepping its own load-bearing noun — the
field, the identifier, the file, the concept — against `admin/pipeline/`,
`data/` and the live documents, because a struck ruling survives in the
documents citing it, not in its own section. 113 rows of 160 had a live
referent and stayed. The other 47 moved to
[`archive/decisions-2026-09-22.md`](archive/decisions-2026-09-22.md),
which also holds the whole file verbatim as it stood at `9ec15d66a`.
**A row was archived as _not binding_, never as _wrong_** — nothing was
reversed, only moved.
...
| Column | Means |
|---|---|
| **id** | the ruling's label in its own scheme. Schemes do not share a namespace — see [§B](#b-the-same-ruling-in-two-places-with-two-wordings) for the two ids that collide |
...
Every row below was verified against the working tree on 2026-09-22:
its home path exists, and its load-bearing noun greps to a file under
`admin/pipeline/`, to a path under `data/`, or to a live document —
`README.md`, `CONTRIBUTING.md`, `admin/pipeline/README.md`,
`admin/pipeline/DESIGN.md`, this file, [`glossary.md`](glossary.md) and
`ideas.md` (which lands with the rest of this change). A hit only under
`docs/archive/` does not count; the archive is not live.

---

## 1. V — v2 overhaul (2026-07-03)

Home: [`archive/specs/2026-07-03-v2-overhaul-design.md`](archive/specs/2026-07-03-v2-overhaul-design.md)

**All eleven rows are archived.** V1–V9 and CP-0/CP-1 ruled the shape
of the overhaul itself — the branch, the checkpoints, the subtractive
start — and not one of their nouns survives in `admin/pipeline/`, in
`data/`, or in a live document. What still binds from this era was
re-taken later and is recorded in
[§9](#9-r--pipeline-consolidation-2026-09-12--09-18--09-21): the
re-runnable pipeline as R1, the fresh Sefaria source as R6. The eleven
rows are in
[`archive/decisions-2026-09-22.md`](archive/decisions-2026-09-22.md).

## 2. D — data architecture (2026-07-08)

Home: [`archive/specs/2026-07-08-v2-data-architecture-design.md`](archive/specs/2026-07-08-v2-data-architecture-design.md)
...
| D14 | 2026-07-08 | `migrate.ts` writes the truth tree once, gets blessed, then retires; `compile.ts` runs forever | — | §6 | superseded → R1 (2026-09-13); struck in the spec 2026-09-14. `migrate.ts` carries the pointer at [lines 460–468](../admin/pipeline/migrate.ts) — "NOT because the migration is a one-shot — R1 withdrew that" — and names D14 there as withdrawn. Only the empty-tree guard survives, and R11 schedules its retirement |

## 3. B — entry body model (2026-07-11)

Home: [`archive/specs/2026-07-11-entry-body-model-design.md`](archive/specs/2026-07-11-entry-body-model-design.md)
...
| B11 | 2026-07-11 | The entry format ships as a machine-readable JSON Schema, which **is** the spec; the documents only illustrate | nothing identified | §8 | live (`../data/schema/entry.schema.json`) |
...
| 08-15 cadence | 2026-08-15 | Per-batch escalation review is waived; all `needs_*` rows accumulate into one consolidated report at the end of the sweep | the per-batch signal that the escalation queue is drifting. Batch-01 sampling was the only calibration | [`../admin/pipeline/patch/records/reviewed/README.md`](../admin/pipeline/patch/records/reviewed/README.md) | live (lifted from the archived RUNBOOK) |
| 08-15 triage | 2026-08-15 | Every escalation defaults to `post-go-live`; `blocking` is a per-item override applied during the consolidated review | the default ships every escalation. Nothing blocks until someone overrides it item by item | `patch/records/reviewed/README.md` | live |
| 08-15 wrong-reference | 2026-08-15 | OCR-level reference errors are fixed and relinked; print-level bad references are delinked with an apparatus note; pure linker overreach is delinked **silently** | the silently-delinked class leaves no record that Sefaria ever had a link there | `patch/records/reviewed/README.md` | live |
...
| 08-22 inference | 2026-08-22 | Inference is not transformation: `abbrev-in-alt-headwords` (2,035 entries) reclassifies to `judgment` after its transform was written and matched | 2,035 entries' abbreviated alternates stay unexpanded and are not search keys. The "65.5% resolvable" figure measured consonant uniqueness, not correct pointing | [`archive/specs/2026-08-22-transform-module-design.md`](archive/specs/2026-08-22-transform-module-design.md) §5.2 | live |
...
| 08-24 gershayim | 2026-08-24 | The transform owns the gershayim defect; cite-escape class 1 is retired | the `&quot;` spelling leaves the corpus, so which addresses were once escaped survives only in the 21 respelled obligations | [`archive/specs/2026-08-24-gershayim-transform-design.md`](archive/specs/2026-08-24-gershayim-transform-design.md) §5 | live |
...
| 08-26 link-target gate | 2026-08-26 | The `link-target.ts` gate is strengthened rather than weakened; every refusal is a **gap** with its own case spec | throughput: each refusal costs a spec | [`archive/specs/2026-08-27-link-target-gate-cases.md`](archive/specs/2026-08-27-link-target-gate-cases.md) | live |
| 08-27 paren strip | 2026-08-27 | On `parenthesized-alt-headword`: **strip the delimiters, add no new form-object mark** | **which forms print bracketed.** The parens group a run of variant readings; after the strip nothing records that the run was a group. This is issue #104's worked example — the spec noted the grouping "is not preserved" without saying what that means | [`../admin/pipeline/transform/rules/headword.ts`](../admin/pipeline/transform/rules/headword.ts) line 135; [`archive/specs/2026-08-27-headword-field-integrity-design.md`](archive/specs/2026-08-27-headword-field-integrity-design.md) §3.2, §7.1 | **reversed in intent → HW-paren (2026-09-20)**; the rule was unregistered 2026-09-21 with the §2 adoption |
| 08-28 stranded stem | 2026-08-28 | The stranded shared stem head is **withdrawn to `judgment`**, not discarded: the data is complete, what is left is a Phase 4 rendering decision | a reader meeting a bare `Pa. בַּהַית` cannot tell it shares the next gloss. `entry.schema.json` has no way to say two stems share one | [`archive/specs/2026-08-28-structural-repairs-design.md`](archive/specs/2026-08-28-structural-repairs-design.md) §5 | live |
...
| 09-01 case 9 | 2026-09-01 | Write link-target gate case 9 and ship all four | nothing identified | [`archive/specs/2026-09-01-link-target-gate-case-9.md`](archive/specs/2026-09-01-link-target-gate-case-9.md) | live |
| 09-06 page source | 2026-09-06 | The hOCR page index is the source of page and column. The v1 locators and the 107 hand edits are a cross-check only | the standing review gate on the 289 print-locator fixes retires with them. They are no longer applied, only compared | [`archive/specs/2026-09-06-migrate-design.md`](archive/specs/2026-09-06-migrate-design.md) §1 | live (supersedes D-architecture §6 rule 6 and part of S7 #39) |
| 09-06 case 10 | 2026-09-06 | Link-target gate case 10 splits into three classes, each ruled separately; a second remedy the same day | unstated | [`archive/specs/2026-09-06-link-target-gate-case-10.md`](archive/specs/2026-09-06-link-target-gate-case-10.md) | live |
...
Home: [`archive/specs/2026-09-13-pipeline-consolidation-design.md`](archive/specs/2026-09-13-pipeline-consolidation-design.md) §2

| id | date | decision | drops | home | status |
|---|---|---|---|---|---|
| R1 | 2026-09-12 | `migrate.ts` is permanent and re-runnable. D14's "retires into repo history" is withdrawn. A run regenerates a candidate tree and reports; it never silently overwrites edited truth | nothing identified — it recovers reproducibility | §2 | live, in prose **and** in code: `migrate.ts` lines 460–468 state it and `migrate.test.ts`'s "the `--write` guard (spec R1, permanent)" tests it. The empty-tree guard that outlived D14 is documented there as a stand-in for the §3.2 merge, not as a one-shot |
...
| HW-equals | 2026-09-20 | `=` in a headword (2 rows) is a source defect: `= Y` moves into the gloss, matching the print | nothing — the targets resolve (A00477 alt, E00628). `slugStem` needs no `=` case | §4 | decided; blocked in [#113](https://github.com/UniquePixels/jastrow/issues/113) — no op moves text into a gloss. Half the drops column is spent: `slugStem` went with the slug scheme on 2026-09-21 (U2, and [`glossary.md`](glossary.md) § Retired terms), so the `=` case it needed is moot — what still binds is the `=` in `text`, which rule 4 reports |
...
Home: [`archive/specs/2026-09-21-url-names-design.md`](archive/specs/2026-09-21-url-names-design.md) §2
...
| Scheme | Era | Kept | Archived |
|---|---|---|---|
| V*n* / CP-*n* | v2 overhaul, 2026-07-03 | 0 | 11 |
| D*n* | data architecture, 2026-07-08 | 8 | 7 |
| B*n* | entry body model, 2026-07-11 | 10 | 1 (B2/B3 were taken together) |
| S*n* | sense structure repair, 2026-08-06 | 2 | 5 |
| RP*n* | research process, 2026-08-10 | 5 | 3 |
...
**This table was itself stale before the prune.** It claimed 36 `HW-*`
rows, 13 `R*` rows and 157 in all; the file held 38, 14 and 160. The
three `HW-*`/`R*` rows taken on 2026-09-21 and 2026-09-22 were added to
their sections without the tally being moved. The numbers above are
counted from the sections, not carried forward.

## B. The same ruling in two places with two wordings

Seven rulings or ids are recorded more than once and say different things. Post-consolidation review [§8](archive/specs/2026-09-21-post-consolidation-review.md) and the architecture report [§5](archive/review-2026-09-21/report-architecture.md) list the first five; the sixth is an id collision found while building this index, and the seventh one found while pruning it on 2026-09-22.

1. **The slug freeze — three scopes.** D12 says the slug is *frozen at import*; R10 says a *published* slug never changes and binds at v2 publication; U6 says a published *name* may change but never points at a different entry. Three different promises. **Closed 2026-09-22.** The slug, `SLUGS_FROZEN` and `data/slug-index/` went in code on 2026-09-21 (URL names §9 steps 1–3); D12 and R10 left the live ledger here, so U6 is now the only promise written down. Both superseded rows are in [the archive](archive/decisions-2026-09-22.md).
2. **No-new-text — four layers, four wordings.** RP4 ("rearrange, re-tag, split or delete — never generate new words"); the transform module's three-layer gate ([§5](archive/specs/2026-08-22-transform-module-design.md)); the validator's own docstring in `patch/no-new-text.ts`; and the 2026-09-18 "an agent patch can not add/remove, a human patch can." The fourth forbids what the first permits. The consolidation spec §4.2 flags this itself: 7 `delete` patches sit in the accepted corpus, and the remove half is left to the maintainer.
...
4. **`migrate.ts`'s lifetime.** **Closed 2026-09-22, and the line numbers were wrong.** `migrate.ts` is 638 lines, so there is no 673–685, and nothing in it claims a one-shot: lines 460–468 carry the supersession — "NOT because the migration is a one-shot — R1 withdrew that" — naming R1, R11 and the withdrawn D14. What survives is the empty-tree guard alone (`refuseUnlessEmpty`, called from `main`), documented in that same docstring as a stand-in for the unbuilt §3.2 merge. D14 and R1 now point at it instead of at each other.
5. **The parentheses.** **Closed 2026-09-21.** The 2026-08-27 ruling stripped them; HW-paren keeps them as structure in `display`. The code now does the second: `parenAltHeadword` is UNREGISTERED, and `transform/registry.ts` says so where the rule sat. Both rows stay below, each pointing at the other, so the direction of the reversal is readable from either end.
6. **`D3` named two different rulings.** **Closed 2026-09-22.** A v1-era "D3 — the pipeline is a one-time relic, never re-run" was superseded by the v2 overhaul; the data-architecture spec's own D3 was "transform once." Same id, two rulings, five days apart, disambiguated nowhere. Both are now only in [the archive](archive/decisions-2026-09-22.md) — the second because neither of its halves greps to a live file: R6 carries "a Sefaria schema change is a code change" (verbatim at `DESIGN.md:303`), and the loss of field-by-field comparison with the export is recorded at `DESIGN.md:180` and `:814` ("the shape no longer corresponds 1:1 to the source's split items"). No `D3` is live, so the collision cannot be read wrongly.
7. **`R*n*` names two different schemes.** The `R*` rows in [§9](#9-r--pipeline-consolidation-2026-09-12--09-18--09-21) are the consolidation rulings, but the transform-era batch specs numbered their own rulings `R1`, `R2`… and live code cites both. `transform/rules/seam-space.ts` and `rules/italic-paren.ts` cite a "ruling R2" about declared copies; `transform/abbrev-vocab.ts` and `registry.ts` cite a "ruling R1" about granularity. Neither is the row of that name below. Read an `R` citation together with the file it sits in.
