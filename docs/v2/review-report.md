...
**What to do:** Patch the headword so the derived name carries no URL-unsafe character.
...
**What to do:** Nothing to do: a multi-word form is legitimate (headword-design §4); the rows are listed so the count stays visible.
...
**What to do:** Re-run `bun data:import --write` and commit the entry tree, or the committed slugs disagree with this run.
