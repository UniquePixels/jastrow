...
| 14 | Plural sections (separate lemma sets in print) flattened into the preceding sense's definition; no structure for them in the model | all `Pl.` entries (5,484); visibly damaging in the 5 with restarted `1)…` numbering (A01047, B01292, C00062, D00194, E00789 — the census's 25 candidates include 20 citation-chapter-number lookalikes, hand-verified) | C00062 (`—Pl. גְּבוּרוֹת 1)…2)…` inside sense —3's text) | maintainer print verification 2026-07-13 (B12); plural.ts paren-balance discriminator | to report |
