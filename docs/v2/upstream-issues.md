...
| 16 | Implied sense `1)`: when sense 1 is only a cross-reference after the grammatical label, print omits the `1)` and the next sense opens `—2)` — transcription is print-faithful, but the data gives consumers a sequence starting at 2 | 39 occurrences | B00991 (`(v. preced. 2)`), B01321, D00072 | [body-review 01](body-review/01-broken-sequences.md) Note 1; maintainer print verification 2026-08-05 | to report (informational — matches print; v2 inserts the `1)` itself as a recorded deviation) |
