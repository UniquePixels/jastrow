...
class SagesData {
  ...
  async load() {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load sages data: ${response.status} ${response.statusText}`);
    }
 ...
 */
SagesData.formatDates = function (dates) {
  if (!dates) return "";
  const b = dates.born < 0 ? `${Math.abs(dates.born)} BCE` : `${dates.born} CE`;
  const d = dates.died < 0 ? `${Math.abs(dates.died)} BCE` : `${dates.died} CE`;
