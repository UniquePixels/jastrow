...
class TalmudSagesExplorer {
  ...
  async _initialize() {
    ...
    this._graphContainer.addEventListener("click", (e) => {
      ...
      if (e.target === this._graphContainer ||
    ...
    this._toolbarContainer.querySelectorAll(".rel-filter").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (activeRelTypes.has(rel)) {
          if (activeRelTypes.size <= 1) return;
  ...
  _buildZoomControls() {
    ...
    for (const [cls, iconName, iconLabel] of [["zoom-in", "plus", "Zoom in"], ["zoom-out", "minus", "Zoom out"], ["zoom-fit", "arrows-maximize", "Fit to view"]]) {
      const btn = document.createElement("wa-button");
      btn.className = cls;
      btn.setAttribute("appearance", "outlined");
      btn.setAttribute("size", "small");
    ...
    controls.querySelector(".zoom-in").addEventListener("click", () => this.graph.zoomStep("in"));
    controls.querySelector(".zoom-out").addEventListener("click", () => this.graph.zoomStep("out"));
    controls.querySelector(".zoom-fit").addEventListener("click", () => this.graph.fitToView());
  }
  ...
  _buildLegend() {
    ...
    const items = [
      ...
      { line: "dashed", lineColor: "#6b7280", label: "Sibling", dashStyle: "2 4" },
    ];
