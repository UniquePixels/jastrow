...
class SagesGraph {
	...
	_calculateLayout() {
		...
		const rows = [];
		for (let g = 1; g <= 5; g++) {
			rows.push({ era: 'tanna', generation: g });
		}
		for (let g = 1; g <= 7; g++) {
			rows.push({ era: 'amorai', generation: g });
		}
		...
		for (const row of rows) {
			...
			currentY +=
				Math.max(
					this.ROW_HEIGHT,
					cellRows * (this.NODE_HEIGHT + this.NODE_GAP),
				) + 8;
		}
		...
		for (const [key, group] of bands) {
			...
			if (key.endsWith('-bavel')) {
				maxBavel = Math.max(maxBavel, nodesPerRow);
			}
			if (key.endsWith('-israel')) {
				maxIsrael = Math.max(maxIsrael, nodesPerRow);
			}
		}
		const bavelWidth = Math.max(
			maxBavel * (this.NODE_WIDTH + this.NODE_GAP),
			300,
		);
		const israelWidth = Math.max(
			maxIsrael * (this.NODE_WIDTH + this.NODE_GAP),
			300,
		);
		...
		for (let i = 0; i < yearToGen.length - 1; i++) {
			if (year >= yearToGen[i].year && year < yearToGen[i + 1].year) {
				const row = this._rows.find(
					(r) =>
						r.era === yearToGen[i].era &&
						r.generation === yearToGen[i].generation,
				);
				const nextRow = this._rows.find(
					(r) =>
						r.era === yearToGen[i + 1].era &&
						r.generation === yearToGen[i + 1].generation,
				);
				if (row && nextRow) {
					const t =
						(year - yearToGen[i].year) /
						(yearToGen[i + 1].year - yearToGen[i].year);
	...
	_drawNodes() {
		...
		for (const sage of sages) {
			...
			const node = g
				...
				.attr(
					'class',
					`node node-${sage.era} node-${sage.location}${sage.nasi ? ' node-nasi' : ''}`,
				)
			...
			node
				...
				.attr(
					'aria-label',
					`${sage.name.en}, ${sage.name.he}, ${this._formatDates(sage.dates)}`,
				);
	...
	selectSage(id) {
		...
		this.zoomGroup.selectAll('.node').each(function () {
	...
	panToSage(id) {
		...
		const ty =
			containerRect.height / 2 - (pos.y + this.NODE_HEIGHT / 2) * scale;
	...
	setFilters({ activeGroups, activeRelTypes }) {
		...
		if (activeRelTypes !== undefined) {
			this._activeRelTypes = activeRelTypes;
		}
		this.render();
	}
