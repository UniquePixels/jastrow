...
class SagesSidebar {
  ...
  open(sageId) {
    ...
    // Teachings
    if (sage.teachings?.length) {
      this.container.appendChild(this._buildListSection("Key Teachings", sage.teachings));
    }

    // Stories
    if (sage.stories?.length) {
      this.container.appendChild(this._buildListSection("Notable Stories", sage.stories));
    }
  }
  ...
  _buildHeader(sage) {
    ...
    eraBadge.setAttribute("variant", sage.era === "tanna" ? "brand" : "success");
    ...
    for (const [type, targets] of grouped) {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        link.addEventListener("click", (e) => {
          e.preventDefault();
          if (this.onNavigate) this.onNavigate(targets[i].id);
        });
        group.appendChild(link);
        if (i < targets.length - 1) group.append(", ");
      }
    ...
    try {
      if (navigator.share) {
        ...
        if (toast) {
          ...
          toast.create("Link copied to clipboard", { variant: "success", icon: "check", duration: 2000 });
        }
      }
    } catch (err) {
