...
class JastrowDataLoader {
	...
	async checkVersion(db) {
		...
		try {
			...
			const response = await fetch(IDB.VERSION_URL, {
			...
			if (window.DEBUG) {
				console.log('[DataLoader] version.json fetch failed (offline?)');
			}
		}
	...
	async writeToCache(db, entries, version) {
		...
		if (window.DEBUG) {
			console.log(
				`[DataLoader] Cached ${entries.length} entries, version: ${version}`,
			);
		}
	}
	...
	async load(progressCallback) {
		try {
			...
			try {
				...
				if (!versionCheck.needsNetwork) {
					...
					if (cachedEntries.length > 0) {
						...
						if (window.DEBUG) {
							console.log(
								`[DataLoader] Loaded ${cachedEntries.length} entries from IndexedDB`,
							);
						}
					}
				...
				if (window.DEBUG) {
					console.warn(
						'[DataLoader] IndexedDB unavailable, falling back to network:',
						idbError,
					);
				}
				db = null;
			...
			if (!loadedFromCache) {
				...
				if (db) {
					try {
						...
						if (window.DEBUG) {
							console.warn(
								'[DataLoader] Failed to write to IndexedDB cache:',
								writeError,
							);
						}
			...
			const normMap = new Map(
				refArray.map((r) => [r, this.normalizeReference(r)]),
			);
			refArray.sort((a, b) => {
	...
	async loadFile(dataUrl, fileIndex, progressCallback) {
		...
		while (true) {
			...
			const overallProgress = Math.min(
				((fileIndex + fileProgress) / numFiles) * 100,
				100,
			);
	...
	binarySearchClosest(query) {
		...
		if (
			left < this.sortedHeadwords.length &&
			this.sortedHeadwords[left] >= query
		) {
			return left;
		}
	...
	searchHeadwordPrefix(query, limit = 8) {
		...
		for (
			let i = left;
			i < this.sortedHeadwords.length && results.length < limit;
			i++
		) {
	...
	searchReferencePrefix(query, limit = 8) {
		...
		for (
			let i = left;
			i < this.normalizedReferences.length && results.length < limit;
			i++
		) {
	...
	searchByReferencePrefix(query) {
		...
		return Array.from(entryIndices)
			.sort((a, b) => a - b)
