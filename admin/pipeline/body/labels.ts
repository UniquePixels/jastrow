 ...
 * for the same continuation marker). Both quarantine to `{unknown}`
 * rather than being silently coerced into the em-dash form, since
 * coercing would break byte-exact regeneration for those five source
 * strings.
 *
 ...
 * (the corpus census found zero letter labels here, and `lettered.ts`'s
 * synthesized sub-item labels are lowercase-only).
 */
...
// label body — one or more digits, or a single lowercase letter. No
// other character survives between the optional prefixes and the
// closing paren; anything else (bracket, ASCII hyphen, missing paren,
// bare number with no paren) fails to match and quarantines to
// `{unknown}`.
const LABEL = /^(?<dash>—)?(?<star>\*)?(?<label>\d+|[a-z])\)$/u;

/** Parse a raw source `sense.number` token into its normalized parts, or
 * `{unknown: raw}` when the token falls outside the measured print-form
 * grammar (quarantine — never guess at a value not seen in the corpus;
 * mirrors `grammar.ts`'s `parseMarker` convention). `raw` is returned
 * untrimmed, exactly as received. */
function parseLabel(raw: string): ParsedLabel | { unknown: string } {
