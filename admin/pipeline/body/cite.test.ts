...
describe('findCitations quote-aware open-tag reading', () => {
	// The latent gap `findTagEnd` closes, on this module's old `<[^>]*>`:
	// `OPEN_TAG` closed a refLink open tag at the first `>` even inside a
	// quoted attribute value, so the rest of the attribute read as
	// document text and the hit came back malformed (or, with `class`
	// after the `>`, not at all).
