...
/** The detector over an already-loaded corpus; returns sorted hits.
 * Corpus-tier tests call this with `corpus-fixture.ts`'s
 * `sourceEntries()` rather than re-reading the snapshot. */
function censusOf(entries: readonly SourceEntry[]): string[] {
	const hits: string[] = [];
...
/** Run the detector over the whole corpus; returns sorted hits. The
 * CLI's own reader — every test goes through `censusOf`. */
