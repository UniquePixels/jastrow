...
function render(rows: IssueRow[]): string {
	...
	// different directories (docs/reports/ and docs/v2/, until Task 6
	// moves the design doc into the module), so a hardcoded same-directory
	// link would break the moment either one moves independently.
	...
	const lines = [
		...
		`and the open questions live in [headword-design.md](${designLink}).`,
		'',
