# Sweep Agent Prompt — v2

- **Version:** `v2` — record verbatim in every patch's
  `prompt_version`. Supersedes [sweep-v1.md](sweep-v1.md) (pilot
  prompt); tranche outputs record which version produced them.
...
- **Contract code:** `admin/pipeline/patch/schema.ts` (patch ops),
  ...
  `admin/pipeline/research/verify.ts` (ingest). Where this document
  and that code disagree, the code wins — report the discrepancy.
- **Sign-off:** maintainer, 2026-08-13.
...
| Prompt version | `v2`; copy verbatim into every patch's `prompt_version` |
...
```jsonl
{"id":"P000001","rid":"X00001","target":"sense[]:56b28a8d","op":"split","expected_before":"first meaning.—2) second meaning.","expected_occurrences":1,"occurrence_index":1,"payload":{"marker":"—2)"},"confidence":"high","rationale":"In-text —2) run with no 1) before it; split the swallowed boundary.","defect_class":"implied-one","snapshot":"sha256:<pin>","prompt_version":"v2"}
{"id":"P000002","rid":"X00001","target":"sense[]:75f7e275","op":"retag","expected_before":"first meaning.","expected_occurrences":1,"occurrence_index":1,"payload":{"number":"1)"},"confidence":"high","rationale":"Host sense is the implied 1); insert per register #16 convention.","defect_class":"implied-one","snapshot":"sha256:<pin>","prompt_version":"v2"}
```
...
7. `snapshot` and `prompt_version` (`v2`) on every patch.
