# Sweep Agent Prompt — v3

- **Version:** `v3` — record verbatim in every patch's
  `prompt_version`. Supersedes [sweep-v2.md](sweep-v2.md) (batch-01
...
- **Contract code:** `admin/pipeline/patch/schema.ts` (patch ops),
  ...
  discrepancy.
- **Sign-off:** maintainer, 2026-08-13 (drafted from the batch-01
...
a *see-reference* that goes nowhere else. Vocalization-only
(niqqud) differences between display and target are linker
convention, not class 11; the class-11 signature is a consonantal
one-letter mismatch or a wrong destination.
