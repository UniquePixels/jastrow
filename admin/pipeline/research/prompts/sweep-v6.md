# Sweep Agent Prompt — v6

- **Version:** `v6` — record verbatim in every patch's
  `prompt_version`. Supersedes [sweep-v5.md](sweep-v5.md) (the 2.3
  residue calibration, chunks r00001–r00005); tranche outputs record
...
| `one-consonant-diverge` | the display is no corpus headword but sits one non-final consonant from its target | class 11, `needs_human_judgment` — the unvocalized cousin of `exact-headword-diverge`. Displays the target records in its own `alt_headwords` are excluded by the detector as of 2026-09-04, so a hint here is not that case |
...
**multiset** check, not a substring test — `no-new-text.ts` permits a
pure reordering of the found bytes, and the op table below states it
correctly. Disposition `repaired`, confidence `high`.
