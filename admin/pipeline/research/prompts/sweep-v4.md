# Sweep Agent Prompt — v4

- **Version:** `v4` — record verbatim in every patch's
  `prompt_version`. Supersedes [sweep-v3.md](sweep-v3.md) (batch-01
  re-sweep and batch-02 round-1 prompt); tranche outputs record
...
- **Contract code:** `admin/pipeline/patch/schema.ts` (patch ops),
  ...
  discrepancy.
- **Sign-off:** maintainer, 2026-08-17 (drafted from the batch-02
  threshold breach; signed same day).
...
| `abbrev-mislink` | a geresh-abbreviated display (`אִסְפַּ׳`) that abbreviates *this* entry's headword but links elsewhere | class 11, `needs_human_judgment` — unless the abbreviation demonstrably names the target word rather than the headword |
...
| `roman-numeral-display` | an anchor whose display is a bare Roman numeral, naming no citation | class 11, `needs_human_judgment` (31 corpus-wide, all inspected ones spurious) |
