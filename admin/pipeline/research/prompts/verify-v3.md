# Verification Agent Prompt — v3

- **Version:** `v3` — supersedes [verify-v2.md](verify-v2.md).
...
- **Companion:** the sweep prompt version the batch ran under. Read
  it from the batch's own chunk inputs (`promptVersion`) or from
  `PROMPT_VERSION` in `research/corpus-inputs.ts`, which is the pinned
  source of truth — do NOT rely on a version named here, which goes
  stale on every sweep bump and did (this line said `sweep-v3.md`
  while the 2.3 residue calibration ran under v5). Its defect catalog,
...
One change: the **Companion** line named a fixed sweep version
(`sweep-v3.md`) and had been stale for two bumps — the 2.3 residue
calibration ran under v5 while this document still pointed at v3.
Every verifier in that run reported it. The line now names the pinned
source of truth instead of a version, so it cannot go stale again.
