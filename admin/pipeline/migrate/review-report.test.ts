...
describe('renderReviewReport', () => {
	...
	it('renders the exact document', () => {
		expect(renderReviewReport(sample(), SORTED)).toBe(
			[
				...
				'',
				'Names replace slugs (`docs/archive/specs/2026-09-21-url-names-design.md`): the six slug review kinds are gone, a name collision is a gate failure (§7), and §8 reopens why `headword-unparsed` blocks.',
				'',
				'| Publication | Rows |',
				'|---|---|',
				'| blocks | 1 |',
	...
	it('throws naming the kind when a row was minted after classifyRows', () => {
		const report = sample();
		report.rows.push(lineRow('A00005: late', 'markup-carry'));
		expect(() => renderReviewReport(report, SORTED)).toThrow('markup-carry');
	});
