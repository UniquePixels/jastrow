 ...
 * from the catalogue rather than counted with the run's rows. `path`
 * defaults to `PATTERNS_PATH` (`paths.ts`) rather than reaching for
 * the constant directly, so a test can hand it a fixture instead. */
...
function renderReviewReport(
	...
	return [
		...
		'',
		'Names replace slugs (`docs/archive/specs/2026-09-21-url-names-design.md`): the six slug review kinds are gone, a name collision is a gate failure (§7), and §8 reopens why `headword-unparsed` blocks.', // boundary-ignore: prose citing an archived spec for the reader, not resolving a path
		'',
		'| Publication | Rows |',
		'|---|---|',
